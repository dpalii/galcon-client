...
import {
  GameDetails, IncomingEvents, InputUser, OutgoingEvents, User,
} from './types';
...
function App() {
  ...
  const handleConnect = (_gameId: string) => {
    socket.emit(
  ...
  const handleNewGame = (inputUser: InputUser) => {
    socket.emit(OutgoingEvents.CREATE_NEW_GAME, inputUser, (_gameId: string) => {
      const newUser = {
        ...inputUser,
        id: socket.id,
        isHost: true,
